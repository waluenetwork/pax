    ...
    try:
        ...
        while True:
            ...
            if message_count % 5 == 0:
                template_content = generate_basic_template(message_count)
                template_message = {
                    "Template": template_content
                }
                print(f"📨 Sending ULTRA-BASIC template message #{message_count}")
                print(f"🔧 Template preview: {template_content}")
...
def generate_basic_template(count: int) -> str:
    """Generate ULTRA-BASIC Pax template with ZERO styling - just structure"""
    colors = ["rgb(50, 50, 200)", "rgb(200, 50, 50)", "rgb(50, 200, 50)"]
    color = colors[count % len(colors)]
    
    return f"""<Group>
    <Text text="Ultra Basic Dashboard #{count}"/>
    <Text text="WebSocket Connected"/>
    <Text text="Template Update #{count}"/>
    <Rectangle fill={color}/>
</Group>"""

@app.get("/")
async def root():
    return {"message": "SSR WebSocket Server for Pax Dashboard - BASIC TEMPLATE VERSION"}
